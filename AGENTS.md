...
- **Vault password** — `ansible.cfg` sets `vault_password_file = ../../secrets/.vault_pass` (config-relative; the gitignored file is synced with the repo). One-time migration from the old dotenv VARS (run per target BEFORE the first apply): generate the resolved YAML with the old validator (`python3 lib/vars_validator.py <t>` at the commit before this change), then `grep -v '^_vars_keys:' current_target/vars.yml | ansible-vault encrypt --vault-password-file secrets/.vault_pass --output secrets/VARS.<t>.yml` (the old `vars.yml` is resolved, expanded, stringified — lossless). `${NAME}` references no longer expand: write literal values (the template documents them).
