...
Bash 4+, Task 3.x, Python 3 + python-dotenv (VARS parsing), Docker Compose v2, kubectl (K3s targets), yq, envsubst, jq, curl, openssl, python3. Node.js/npm and `go` for local `renovate` runs (npx + gomod manager). Dev-only: `shellcheck` (`task lint`). Install everything: `task prereqs` (universal; detects apt/dnf/rpm-ostree; Docker from official repos; Task binary pinned from GitHub releases).
...
task <target>:k3s:setup                         # Bootstrap a K3s control-plane node
task <target>:k3s:join -- <ip> <user> [agent|server]
...
## Rollback (from the Task migration)

The migration replaced `infra.sh` + `lib/dispatch.sh` + `lib/env.sh` with `task` + a `lib/common.sh` bootstrap + `lib/vars_validator.py`, and the VARS files moved from bash exports (`VARS.*.sh`) to dotenv (`VARS.*.env`) via a one-time converter (`migrate_vars.py`, deleted after use; migrated values carry the post-bash-expansion text). To roll back: `git revert` the migration commits — the old entry points and the `.sh` VARS templates come back. Keep the old gitignored `VARS.*.sh` files until the `.env` migration is verified on every machine (they are the only non-git state); deleting them is the last step of the migration. Machines without task yet: `bash commands/prereqs.sh` (installs task + python3-dotenv; from then on use `task prereqs`), then verify with `task <target>:validate`.
