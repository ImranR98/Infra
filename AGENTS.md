...
- **srv0** — `base` group: namespaces, nfs-server, host-volumes, csi-driver-nfs, cert-manager, longhorn, geoip, traefik, crowdsec, crowdsec-web-ui, authelia, pvc-backup, ntfy, descheduler, system-upgrade, monitoring, generic-device-plugin, node-feature-discovery, node-facts. `apps`: immich, logtfy, jellyfin, navidrome, mdscl, mosquitto, homeassistant, ollama, open-webui, nextcloud, freshrss, linkwarden, opodsync, dscpln, opencanary, flaresolverr, fmd, plik, syncthing, headlamp, frigate (see `k3s-base/templates/` + `k3s-apps/templates/`). Compose sidecar = frpc only. Ollama runs on the `bigpc` agent (RX 9070/ROCm); Open WebUI reaches it in-cluster only (no LAN exposure). Jellyfin/Immich ML/Frigate use srv0's Iris Xe iGPU; Frigate recordings stay on NFS deliberately so the pod can move nodes. Home Assistant integration auto-installs on every pod start (no HACS).
- **bigpc** — K3s agent (the AMD GPU is advertised by the generic-device-plugin as `infra.local/amd`), tainted `scheduling-discouraged` (PreferNoSchedule), no Longhorn replicas (`create-default-disk=false`); Compose: dockerproxy_priv, watchtower (syncthing only), syncthing (host net).
- **vps0** — Compose: frps, traefik, authelia + db, crowdsec, crowdsec-web-ui, plausible (app/ClickHouse/Postgres/state-fixperms), shlink + db + web UI, uptime-kuma, dozzle (container health/logs, read-only via the dockerproxy socket), metube, isbn-lookup, pixelntfy, logtfy, strelaysrv, owncast (+ owncast-auth), ikom, cct26, moving-sale, obtainium, socket proxies. No watchtower — all vps0 images are Renovate-owned (floating tags digest-pinned). Two domain zones: `$BASE_SERVICES_DOMAIN` (public) and `$CLOUD_SERVICES_DOMAIN` (Authelia-protected, e.g. `cloud.$BASE_SERVICES_DOMAIN`).
...
## Fresh bootstrap (in order)

Ordering constraints for a from-scratch setup (commands in Essential commands below):

1. **DNS + FRP certs first** — `$SERVICES_DOMAIN` points at vps0, plus `home.$SERVICES_DOMAIN` and `*.home.$SERVICES_DOMAIN` for the tunnel; generate the FRP mTLS certs (openssl commands in the FRP sections of the templates). srv0's cert-manager is HTTP-01, so no srv0 certificate issues until the public path through vps0/frps exists.
2. **`scripts/prereqs.sh` on every machine** — packages (Longhorn iSCSI/NFS included), helm, Docker, machine-fact `.env`, bind dirs.
3. **Config** — `cp -r targets/<t>/config_template config/<t>` (fill every value) plus `cp config_template/VARS.env config/VARS.env`; set `INFRA_ROOT`, `PVC_BACKUP_DIR`, `MAIN_PARENT_DIR`, `MEDIA_DIR_PATH`, etc. to the machine layout.
4. **vps0 before srv0** — `validate.sh vps0` + compose up: the edge must be live for srv0's ACME challenges.
5. **srv0** — `validate.sh srv0` → `k3s-server.sh` → `kubeconfig-unlock.sh` → the frpc compose sidecar → `helm upgrade --install srv0-base` → `srv0-apps`. On a fresh bootstrap set `AUTHELIA_HEADER_GATE_ENABLED: "true"` until Authelia is Ready, then back to `"false"`.
6. **Join nodes** — `k3s-join.sh` from the control plane (after `srv0-base`, so Longhorn exists for the replica-count step).
7. **Machine-local extras** — `wireguard.sh`, `preboot.sh`.
8. **Post-deploy** — the manual first-time setup steps below (app seeding, crowdsec-web-ui machine registration).

The in-cluster jobs (`pvc-backup`, `renovate`) mount the repo from `INFRA_ROOT`; the repo can live anywhere (it does not need to be under `MAIN_PARENT_DIR/Main`).
...
bash scripts/k3s-join.sh <node_ip> <node_user> \
  [--role agent|server] [--scheduling-discouraged] [--longhorn-replicas]   # run ON the control plane
...
| `scripts/prereqs.sh` | — | this machine | packages (incl. Longhorn iSCSI/NFS + `iscsid`), pinned helm, Docker bootstrap, machine-fact `.env`, host bind dirs, acme seed |
...
| `scripts/k3s-server.sh` | — | this machine (becomes the control plane) | node prep + installer + server config + Longhorn default-disk label |
| `scripts/k3s-join.sh <ip> <user> [...]` | — | control plane + the joining node | token travels via stdin over ssh; taint/Longhorn after Ready |
| `scripts/k3s-node-prep.sh` | — | the node (root) | sysctls, firewall — shared by server/join |
...
- **Universal VARS** — target-agnostic secrets live in `config/VARS.env`, dotenv format (template: `config_template/VARS.env` — `cp config_template/VARS.env config/VARS.env`). Loaded on demand by the commands that need them (`scripts/renovate.sh` requires `RENOVATE_GITHUB_TOKEN`; the in-cluster renovate CronJob does `set -a; . /repo/config/VARS.env`). Not template-validated — each command checks its own variables.
...
- `k3s-server.sh` (no args, run ON the node): `k3s-node-prep.sh` (sysctls via `/etc/sysctl.d/90-k3s.conf`, firewall (firewalld/ufw; K3s ports incl. 51820–21/udp for flannel-wg)) + the installer + `/etc/rancher/k3s/config.yaml` (`selinux: true`, `write-kubeconfig-mode: "0600"`, `flannel-backend: wireguard-native`, the flannel-iface regex, `cluster-init: true`, `node-ip`) + restart + waits for the server token and the API + applies the Longhorn default-disk label. Node feature labels are NFD-managed (see the Security section). K3s needs a fixed IP — if it changes, run `scripts/update-node-ip.sh`.
- `k3s-join.sh <node_ip> <node_user> [...]` (run ON the control plane; prompts for sudo on the control plane and the joining node separately — they may differ): reads the token from `/var/lib/rancher/k3s/server/token`, resolves the server IP, streams `k3s-node-prep.sh` + the installer to the node over SSH with the token appended on stdin (never argv; the token lands in the node's root-only `k3s-agent.service.env` for agents, or in `config.yaml` for joined servers), waits for the node to appear + become Ready, then applies the taint/Longhorn replica count. All join options are CLI flags: `--role agent|server`, `--scheduling-discouraged` (PreferNoSchedule taint), `--longhorn-replicas` (`create-default-disk=true` + auto-increment of `default-replica-count`, guarded by the label's actual change so re-runs don't double-count; without the flag → `create-default-disk=false`). The installer only runs when k3s is missing — re-provision never re-installs, so it can't fight system-upgrade-controller's ownership of versions. SSH host-key checking is left at the default (on) — the first join prompts to accept the fingerprint, same trust model as plain `ssh`.
...
2. **k3s's embedded helm-controller** → the 20 `HelmChart` CRs *inside* our charts. Our releases apply the CR objects; the controller then installs/upgrades the app releases (frigate, immich, …). `helm uninstall srv0-*` does NOT touch these; `kubectl delete helmchart` triggers THEIR uninstall.
...
- **Longhorn** is the default StorageClass and primary backend (replica count 1, best-effort locality, 2000% over-provisioning; minor upgrades must be sequential). NFS (`nfs-server` + `csi-driver-nfs`) and static hostPath PV/PVC pairs (`host-volumes`, RWX, bound to the `infra.local/hostpath-main` label) remain for shared host data. The `longhorn-manager` warning `Failed to get filesystem device type of /var/lib/longhorn/` is expected on LUKS nodes (device-mapper volumes appear in sysfs as `dm-N`, not by their `luks-*` name); it fires only on disk-count reconciliation and is harmless.
- **Backup** — `pvc-backup` (base group) is a nightly 3AM CronJob that runs `scripts/pvc.sh backup --all -y` in-cluster (bitnami/kubectl:latest, hostPath mounts of `$INFRA_ROOT` + `$PVC_BACKUP_DIR`, nodeSelector `infra.local/hostpath-main`). PVCs labelled `auto-backup: "true"` are archived; annotation `backup.infra/exclude` adds tar `--exclude` patterns. A temp pod (scheduled on the volume's node for RWO; tolerates `scheduling-discouraged`) tars the live PVC (no scale-down) to the shared `pvc-backup-dest` PVC — a static PV bound to the ROOT of the NFS backups share (= `$PVC_BACKUP_DIR`), so archives land directly at their final human-named path `<name>.tar.gz` (with `__backup_timestamp.txt` inside), overwritten each run, reachable from any node.
...
- **Home Assistant gets the ConBee II via the generic-device-plugin, not `privileged`** (`targets/srv0/k3s-base/templates/generic-device-plugin.yaml` + `k3s-apps/templates/homeassistant.yaml`) — the device cgroup blocks unprivileged opens of `/dev/ttyACM0`, so HA requests the `infra.local/conbee` resource. The `generic-device-plugin` DaemonSet (squat/generic-device-plugin) discovers each node's `/dev` (grants follow the hardware, not a node label) and advertises stable path-derived IDs; nodes without a matching device register zero. HA still needs pod-level `seLinuxOptions.type: spc_t` (SELinux denies `container_t` the mounted `/run/dbus/system_bus_socket` — Bluetooth integration → host bluez — and the device; verified empirically) plus `capabilities.add: [NET_ADMIN, NET_RAW]` (habluetooth manages the host adapter via direct HCI sockets; since hostNetwork was removed — auto-discovery unused — these apply to the pod netns only). Device cgroup access is scoped to the ConBee II alone. Don't propose re-adding privileged.
- **Node labels are NFD-managed** (`targets/srv0/k3s-base/templates/node-feature-discovery.yaml` + `node-facts.yaml`) — NFD (chart, namespace `node-feature-discovery`) applies the `node-facts` DaemonSet's local feature files as node labels: `infra.local/hostpath-main` (`$MAIN_PARENT_DIR/Main` exists) and `infra.local/external-exposed` (an `frpc` process is running). The detector re-evaluates every 60s and writes atomically into the shared `features.d` host dir (an omitted fact removes its label); it mounts only host `/proc` read-only (host root reached via `/proc/1/root`, no host `/` mount) with pod-level `spc_t` + `SYS_PTRACE` for the ptrace LSM checks and `DAC_OVERRIDE` to traverse the 0700 home directory. NFD has no file/process feature source, so path/process facts cannot come from rules — the detector is NFD's documented "external feature detector" extension point. Hardware scheduling is owned by the generic-device-plugin (`infra.local/amd`, `infra.local/conbee`), so NFD carries no hardware rules; its built-in PCI/USB labels still appear incidentally. `external-exposed` follows the live frpc process: logtfy restarts during an frpc outage go Pending and self-schedule when frpc returns (running pods are never evicted — PV node affinity is scheduling-time only). The NFD chart's CRDs are install-only (Helm `crds/`), and no NFD CRs are managed. Removing the detector leaves the last feature file behind — clean it with `rm /etc/kubernetes/node-feature-discovery/features.d/node-facts`.
- **Jellyfin gets /dev/dri via the generic-device-plugin, not `privileged`** (`targets/srv0/k3s-apps/templates/jellyfin.yaml`) — requests `infra.local/dri`, runs `container_t` as `$MY_UID`, no spc_t needed (renderD128 is 0666 + container_t-accessible). Device cgroup scoped to the render node. frigate/immich get the same grant (container_t + s0, pinned to srv0 — frigate keeps CAP_PERFMON but loses the dashboard GPU-stats graph: SELinux denies container_t perf_event_open); the device plugin runs unprivileged (spc_t, no capabilities); pvc-backup/restore pods are non-privileged (spc_t + DAC_OVERRIDE/FOWNER); ollama uses the `infra.local/amd` grant (kfd + dri, bigpc) with container_t + s0; mosquitto runs as uid 1883. nfs-server ingress is allowlisted to the node CIDR on TCP 2049 (all shares nfsvers=4.2, CSI mounts are node-sourced); that CIDR is hardcoded to `192.168.8.0/24` in `nfs-server.yaml` — update it for a different LAN.
...
- **crowdsec-web-ui** (srv0 + vps0, one-time): register the LAPI machine with the `CROWDSEC_WEBUI_LAPI_PASSWORD` value, then create the initial admin account in the UI (`https://crowdsec.$SERVICES_DOMAIN` / `https://crowdsec.$CLOUD_SERVICES_DOMAIN`). srv0 (during an unlock): `kubectl -n base exec deploy/crowdsec-lapi -- cscli machines add crowdsec-web-ui --password '<value>' -f /dev/null`. vps0: `docker exec crowdsec cscli machines add crowdsec-web-ui --password '<value>' -f /dev/null`. Registration persists in the LAPI DB (`crowdsec-pvc` / `compose_live_state/crowdsec/data`); the app retries bootstrap in the background so deploy order doesn't matter.

## Updates (Renovate)

Renovate runs **automatically every day at 17:00 America/Toronto** as the `renovate` K3s CronJob on srv0 (base group, `targets/srv0/k3s-base/templates/renovate.yaml`) — the full `renovate/renovate` image (ships the Go toolchain, so the gomod manager works). The pod mounts only two files of the synced repo (`{{ INFRA_ROOT }}/config/VARS.env` and `{{ INFRA_ROOT }}/.git/config`, read-only) to source `RENOVATE_GITHUB_TOKEN` (auto-rotates on sync) and to infer `RENOVATE_GIT_AUTHOR` — the identity must be set **repo-locally** (`git config user.name/user.email`), since a global-only `~/.gitconfig` is invisible to the pod and a default Renovate author makes the next run treat its own branches as foreign (autoclose skipped, rebases blocked); Renovate itself clones from GitHub. `bash scripts/renovate.sh [--dry-run]` is the manual/on-demand equivalent for other machines (needs Node/npm and `go` from prereqs for gomod updates; runs on the machine you're on, opening PRs directly on GitHub).

Review/apply flow (every PR is manual): fetch the PR branch (`git fetch origin pull/<n>/head:renovate/pr-<n>`, then `git checkout renovate/pr-<n>`), `bash scripts/validate.sh <target>`, then merge locally and `git push origin master`. Merges never happen in the platform UI — origin stays the source of truth. Renovate rebases its open PRs and auto-closes them once the change lands on `master` (next run).

**No automerge** — `automerge: false` is set globally in `renovate.json`; every PR (including patch/minor and the grouped docker-digests batch) waits for manual review and merge. There is no CI gate, and a merged change only deploys when the deploy commands are next run (`docker compose up -d` / `helm upgrade --install`).
...
`renovate.json` at root (repository config): built-in **kubernetes** manager (`managerFilePatterns: /^targets\/srv0\/k3s-(base|apps)\/.*\.ya?ml$/` — plain pod-spec images incl. the hook Jobs and vendored system-upgrade manifests), **docker-compose** (all compose images), **dockerfile** (the geoip base image) and **gomod** (both `go.mod`s). Regex managers: (1) images in the k3s charts (`valuesContent` repository/tag pairs and single-line refs; `currentDigest` is captured so pinned images refresh instead of re-pinning), (2) HelmChart CR versions (`oci://` via the docker datasource — the helm datasource has no OCI support — or `chart:`+`repo:`+`version:`), (3) K3s plan versions (`github-releases` on `k3s-io/k3s`, custom versioning), (4) Traefik plugin pins in `additionalArguments` (also covers vps0 compose), (5) immich-machine-learning (github-releases, preserves the `-openvino` suffix), (6) the HA Frigate integration + Advanced Camera Card pins, (7) dotdc Grafana dashboard URLs in the k8s-monitoring values, (8) images embedded in `scripts/pvc.sh`, (9) `HELM_VERSION` in `scripts/prereqs.sh`. Global options (token, repo) are set by the runner script, not the repo config. packageRules: pin floating `latest|stable|release|alpine` tags (and every untagged compose image, which carries an explicit `:latest`) to digests and group all digest pins/refreshes into one PR (prHourlyLimit 20); block majors for `postgres`, `clickhouse/clickhouse-server`, `fedora`, `helm/helm`; disable syncthing (compose; watchtower-owned on pc/bigpc) and the frozen moving-sale site image; immich's postgres image gets custom regex versioning (same-shape `18-vectorchordX.Y.Z-pgvectorA.B.C` tags only, postgres major locked via the compatibility group). No other annotations — Renovate's default update decision applies everywhere.

Custom-built artifacts are manual on the source side: `targets/srv0/k3s-base/geoip-src/src/build.sh` builds/pushes the geoip image and re-pins its digest in `geoip.yaml` (run after merging a go.mod bump; Renovate still tracks the digest for automated PRs), and a `plugins/authelia-header-gate` gomod bump only changes `go.mod`/`go.sum` — run its `build.sh` and commit the rebuilt `files/plugin.wasm` + `files/traefik-plugin-config.yaml`.
...
- **Apply through the real commands** — `helm upgrade --install` for k3s, `docker compose up -d` for compose (the join-time taint/Longhorn label go through `k3s-join.sh`). Direct inspection (logs, get, describe, curl) is fine.
