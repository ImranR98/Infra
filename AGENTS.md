...
Retained bash scripts (`lib/common.sh` bootstrap: state dirs, `MY_UID`, `retry`/`_confirm`, hostname warning; `targets/<t>/commands/` payloads) exist only where something must run **as a file**: the in-cluster `pvc-backup` CronJob runs `targets/srv0/commands/backup-pvc.sh` from a kubectl pod (no ansible there), the preboot/update-node-ip implementations are deliberately kept as bash under their playbooks, and `commands/renovate.sh` is the runner invoked by `renovate.yml`. Compose VARS live as plain YAML (`secrets/VARS.<t>.yml`, gitignored — no encryption; same at-rest trust model as the old dotenv files) and are loaded natively by the compose role via `include_vars` (flat, for jinja templates, and named `secret_vars`, for the compose interpolation env) when the target has a `VARS.template.yml` (srv0, vps0).
