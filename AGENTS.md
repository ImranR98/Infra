...
Retained bash scripts (`lib/common.sh` bootstrap: state dirs, `MY_UID`, `retry`/`_confirm`, hostname warning; `targets/<t>/commands/` payloads) exist only where something must run **as a file**: the in-cluster `pvc-backup` CronJob runs `targets/srv0/commands/backup-pvc.sh` from a kubectl pod (no ansible there), the preboot/update-node-ip implementations are deliberately kept as bash under their playbooks, and `commands/renovate.sh` is the runner invoked by `renovate.yml`. Compose VARS live in ansible-vault encrypted YAML (`secrets/VARS.<t>.yml`, decrypted via the vault password file `secrets/.vault_pass` configured in `ansible.cfg`) and are loaded natively by the compose role via `include_vars` (flat, for jinja templates, and named `secret_vars`, for the compose interpolation env) when the target has a `VARS.template.yml` (srv0, vps0).
...
- **Two formats**: k3s (srv0) uses **Helm values** — committed defaults `targets/srv0/k3s/values.yaml` (gates + `MY_UID`) and gitignored secrets/config `secrets/values.srv0.yaml` (generated from `secrets/VARS.srv0.yml` during the migration; edit it directly now). Compose targets use **vaulted YAML VARS** (`targets/<t>/VARS.template.yml` documents keys + generation commands; real values in the ansible-vault encrypted `secrets/VARS.<t>.yml`). srv0/vps0 have both (k3s chart + compose sidecar); bigpc/pc/rpi are builtin-var-only.
- **Compose VARS pipeline**: `secrets/VARS.<t>.yml` (vaulted YAML map) is loaded by the compose role via `include_vars` — once flat (keys become jinja template variables) and once as `secret_vars` (the dict for **docker-compose native interpolation**, stringified into `compose_env`). No expansion step: values are final. Completeness is asserted against the template keys and placeholder values (`change_me`/`changeme`/`abc`/`REPLACE_ME`/`<...>`) are rejected — assert messages print key names only. `docker compose` interpolates `$VAR`/`${VAR}` in `compose.yaml`/`compose.private.yaml` natively — no envsubst, no merged render file; `-f` merge is native compose.
- **Vault password** — `ansible.cfg` sets `vault_password_file = ../../secrets/.vault_pass` (config-relative; the gitignored file is synced with the repo). Ansible reads it at startup on every machine — builtin-var-only targets (rpi/pc/bigpc) that never decrypt anything still need the file to exist (a dummy line is fine). One-time migration from the old dotenv VARS (run per target BEFORE the first apply): generate the resolved YAML with the old validator (`python3 lib/vars_validator.py <t>` at the commit before this change), then `grep -v '^_vars_keys:' current_target/vars.yml | ansible-vault encrypt --vault-password-file secrets/.vault_pass --output secrets/VARS.<t>.yml` (the old `vars.yml` is resolved, expanded, stringified — lossless). `${NAME}` references no longer expand: write literal values (the template documents them).
- **Format** — vaulted YAML map `KEY: value`. Multi-line values are literal block scalars (`|`) — indentation is data and splices are verbatim (the vps0 geoblock subset is the one `indent(10, true)`-spliced exception). A bare `$` and `#` are literal — write `$argon2id$...` unescaped. Inline comments (` # ...`) work after single-line values.
...
- **Multi-line vars** (Authelia user DB, JWKS, frigate config, geoblock subset, mTLS PEMs): in `secrets/values.srv0.yaml` they are YAML block scalars at base column 0 spliced via `{{ .Values.X | indent "N" }}`; in the vaulted YAML they are block scalars that jinja2 splices verbatim (or via an explicit `indent` filter).
...
secrets/                        # VARS.<target>.yml (vaulted), VARS.env, values.srv0.yaml,
