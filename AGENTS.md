...
Infra is an Ansible-driven IaC repo for a multi-machine homelab. The single CLI is `ansible-playbook` against playbooks in `ops/ansible/playbooks/` (generic ops, run with `-l <target>`) and `targets/<t>/playbooks/` (target-specific ops, host hardcoded); everything for a target lives in `targets/<t>/`. Shared engine (roles, inventory) sits in `ops/ansible/`, secrets in the gitignored `secrets/`. No build step, no agent: playbooks run on the machine being managed (the committed inventory is `connection=local`), k3s applies go through plain Helm via a target playbook, and VARS validation is a small Python validator (`lib/vars_validator.py`). Audience assumed to know Docker/Compose, Kubernetes, Traefik, Ansible.

## Prerequisites

Bash 4+, Python 3 + python-dotenv (VARS parsing), Docker Compose v2, kubectl (K3s targets), helm (srv0), yq, jq, curl, openssl, python3. Ansible core + `ansible.posix`/`community.general` collections. Node.js/npm and `go` for local `renovate` runs (npx + gomod manager). Dev-only: `shellcheck`, `ansible-lint`, `yamllint`. Install everything: `ansible-playbook ops/ansible/playbooks/prereqs.yml` on the machine to prepare — if ansible-core itself is missing, bootstrap it first (`sudo dnf|apt install ansible-core`), then re-run; the playbook installs packages, the pinned helm binary, the official Docker bootstrap, and the collections declaratively.
...
Each target dir: `VARS.template.env` (committed; documents every required variable + generation commands), optional `compose/` and/or `k3s/`, `playbooks/` (target-specific Ansible playbooks with the host hardcoded) and `commands/` (retained bash payloads that must exist as files — e.g. the in-cluster pvc-backup CronJob calls `targets/srv0/commands/backup-pvc.sh` from a kubectl pod).
...
Generic ops run against any target (`-l <target>`); target-specific ops are invoked by their own playbook path. Run from the repo root; `ANSIBLE_CONFIG=ops/ansible/ansible.cfg` is needed only if you don't `cd` there (the config makes roles/inventory paths config-relative). Docker commands need elevation where the user is not in the docker group — add `-K` (ansible prompts for sudo once) on compose up/restart/backup-state runs.

```
# generic (any target):
ansible-playbook ops/ansible/playbooks/validate.yml -l srv0             # yq + compose config + (per-target k3s) helm lint/template
ansible-playbook ops/ansible/playbooks/compose_install.yml -l srv0      # validate VARS, render templates (jinja2), up -d [-K]
ansible-playbook ops/ansible/playbooks/compose_restart.yml -l srv0 -e compose_service=frpc   # re-render, down+up one service [-K]
ansible-playbook ops/ansible/playbooks/compose_backup_state.yml -l srv0  # [-e backup_remote=user@host:path] [-K]
ansible-playbook ops/ansible/playbooks/prereqs.yml                       # universal: install prerequisites
ansible-playbook ops/ansible/playbooks/renovate.yml [-e renovate_args="--dry-run"]
ansible-playbook ops/ansible/playbooks/wireguard.yml -e wireguard_conf_src=$(readlink -f wg.conf)
ansible-playbook ops/ansible/playbooks/k3s_server.yml                    # bootstrap control plane (no args; run ON the node)
ansible-playbook ops/ansible/playbooks/k3s_join.yml -e node_ip=.. -e node_user=..   [-e k3s_role=agent|server] [-e k3s_amdgpu_mode=auto|yes|no] [-e k3s_scheduling_discouraged=true] [-e k3s_longhorn_replicas=true] [-K]

# target-specific (host hardcoded in the playbook; run ON that machine):
ansible-playbook targets/srv0/playbooks/helm_apply.yml -e helm_scope=base|apps   [-e helm_args="--dry-run"] [-e helm_state=absent]                       # umbrella-chart upgrade --install / uninstall
ansible-playbook targets/srv0/playbooks/pvc_backup.yml  [-e pvc_name=<name> | -e pvc_all=true] [-e auto_yes=true]
ansible-playbook targets/srv0/playbooks/pvc_restore.yml [same args]
ansible-playbook targets/srv0/playbooks/update_node_ip.yml [-e update_node_ip=X] [-e update_node_ip_force=true]
ansible-playbook targets/srv0/playbooks/preboot.yml                       # srv0 frpc initramfs tunnel
ansible-playbook targets/bigpc/playbooks/preboot.yml                      # bigpc crypt-ssh initramfs
```

Playbook args after the playbook name are plain ansible (`-e` extra vars, `--check`, `--tags`); target playbooks assert `hostname == inventory_hostname` so a wrong-machine run fails loudly. Non-interactive PVC confirmations: pass `auto_yes=true` (or run the script directly on the machine). Dev-only checks: `shellcheck $(find lib commands targets -name '*.sh' -not -path '*/plugins/*')`, `ansible-lint -c ops/ansible/.ansible-lint --offline ops/ansible`, `yamllint -c ops/ansible/.yamllint ops/ansible`.
...
**Ansible is the only CLI.** `ops/ansible/playbooks/` holds generic ops (compose, validate, prereqs, wireguard, renovate, k3s provisioning) invoked with `-l <target>` against the committed `ops/ansible/inventory.yml` (`ansible_connection: local` for all five targets — playbooks run **on** the machine being managed). `targets/<t>/playbooks/` holds target-specific ops (helm apply, PVC backup/restore, node-IP update, preboot) with the host hardcoded in the playbook; a hostname `assert` fails loudly on a wrong-machine run. K3s node joins (`k3s_join.yml`) build their node host at runtime with `add_host` — no inventory files for provisioning. `ansible.cfg` (roles_path + inventory) is config-relative, so plain `ansible-playbook ...` works from any CWD with `ANSIBLE_CONFIG=ops/ansible/ansible.cfg` (or by running from `ops/ansible/`).

Retained bash scripts (`lib/common.sh` bootstrap: state dirs, `MY_UID`, `retry`/`_confirm`, hostname warning; `targets/<t>/commands/` payloads) exist only where something must run **as a file**: the in-cluster `pvc-backup` CronJob runs `targets/srv0/commands/backup-pvc.sh` from a kubectl pod (no ansible there), the preboot/update-node-ip implementations are deliberately kept as bash under their playbooks, and `commands/renovate.sh` is the runner invoked by `renovate.yml`. VARS loading is `lib/vars_validator.py <target>` (emits `current_target/vars.yml`, consumed by playbooks via `include_vars`), run as a playbook pre-step for compose install/restart/preboot when the target has a `VARS.template.env` (srv0, vps0).

### Writing a new playbook or script

New compose/validation logic belongs in `ops/ansible/` (generic: role + playbook with `hosts: all` and `-l <target>`) or `targets/<t>/playbooks/` (target-specific: `hosts: <t>` hardcoded + the hostname assert). New ops for a single target should NOT live in `ops/ansible/playbooks/` — put them in that target's folder. A retained bash script goes to `targets/<t>/commands/` (or `commands/` when truly universal, like `renovate.sh`) with a `# DESC:` second line, sourcing `$INFRA_ROOT/lib/common.sh`. Useful helpers in `lib/`: `retry <tries> <delay> <cmd-string>`, `_confirm`, `get_sudo_cmd`, `wait_for_k3s_cluster`, `get_node_ip`.
...
- **Compose VARS pipeline**: `lib/vars_validator.py <target>` (run as a playbook pre-step) checks completeness against the template, rejects placeholders, expands `${NAME}` references (defined earlier in-file only), and emits `current_target/vars.yml` (chmod 600, all values as strings + a `_vars_keys` list). The compose role includes it via `include_vars` (keys become jinja template variables) and rebuilds the dict for **docker-compose native interpolation** (`environment:`) from `_vars_keys`. `docker compose` interpolates `$VAR`/`${VAR}` in `compose.yaml`/`compose.private.yaml` natively — no envsubst, no merged render file; `-f` merge is native compose.
...
1. **Our umbrella chart** (`targets/srv0/k3s/`, the chart root) → releases `srv0-base` + `srv0-apps`, applied by `targets/srv0/playbooks/helm_apply.yml`.
...
**Everyday commands:**
```
ansible-playbook targets/srv0/playbooks/helm_apply.yml -e helm_scope=base    # upgrade --install srv0-base, idempotent
ansible-playbook targets/srv0/playbooks/helm_apply.yml -e helm_scope=apps
ansible-playbook targets/srv0/playbooks/helm_apply.yml -e helm_scope=base -e helm_args="--dry-run"   # preview
...
- **LUKS preboot** — `targets/srv0/playbooks/preboot.yml` + `targets/bigpc/playbooks/preboot.yml` (shared `preboot` role): `lib/check_root_luks.sh` detects LUKS via `lsblk -s`; if present, the role clones `ImranR98/dracut-remote-luks-unlock` and installs a dracut module. srv0 variant: the frpc compose templates are rendered first (mTLS certs), then initramfs frpc tunnels SSH via FRPS on port 8887. bigpc variant: crypt-ssh only, dropbear patched to the same port for direct LAN unlock (ethernet only). After rotating preboot mTLS certs, re-run `preboot.yml`. When adding initramfs networking, verify with `lsinitrd` that firmware actually made it in (drivers don't retry firmware loads after pivot_root).
...
Post-update: `git diff` → `ansible-playbook ops/ansible/playbooks/validate.yml -l <target>` → deploy.
...
```
ops/ansible/                    # Ansible: inventory.yml (connection=local) + ansible.cfg + playbooks/
                                #   (compose_install|restart|backup_state, validate, prereqs, wireguard,
                                #   renovate, k3s_server|join) + roles/ (compose, preboot,
                                #   k3s_prepare|install|configure, prereqs, wireguard)
commands/renovate.sh            # Universal: run Renovate (opens PRs on GitHub) — invoked by renovate.yml
lib/                            # common.sh (index + env bootstrap for retained scripts) → pkg/net/k3s
...
targets/<target>/
  VARS.template.env             # Documents every required variable + generation commands
  ...
  commands/                     # Retained bash payloads (backup-pvc.sh, restore-pvc.sh, pvc.sh,
                                #   update-node-ip.sh — run in-cluster or by the target playbooks)
...
- **K3s node provisioning goes through `k3s_server.yml`/`k3s_join.yml` only** — never raw installers or ad-hoc joins; the playbooks supply the verified-installer and secret-handling setup. Dev-only lint (`ansible-lint`/`yamllint`/`shellcheck`) touches nothing.
