...
```
task <target>:validate                          # helm lint/template (srv0) + YAML + compose config checks
task <target>:list-domains                      # Host(...) domains from IngressRoutes & Compose
task <target>:compose:install                   # Render templates, create host dirs, up -d
task <target>:compose:restart -- <service>      # Re-render templates, down+up one service
task <target>:compose:backup-state [remote]
task <target>:compose:generate-mtls-certs -- <server-target>   # mTLS CA/certs for a client↔server pair
...
Args after `--` reach the script as `{{.CLI_ARGS}}`. Universal tasks also exist under every target namespace (e.g. `task srv0:prereqs`) — they run with the target env set, exactly like the old `./infra.sh <target> prereqs`.
...
[Task](https://taskfile.dev) is the dispatcher. The root `Taskfile.yml` defines universal tasks and `includes:` the per-target Taskfiles (`targets/<t>/Taskfile.yml`) as namespaces (`task srv0:compose:install`). Every task is a one-liner that runs a command script with inline `TARGET=<t> INFRA_ROOT={{.ROOT_DIR}}` (the Taskfile's `env:` key cannot be used: Task ≥3.53 has a bug where an included Taskfile's env randomly wins); user args ride `{{.CLI_ARGS}}`. `task --list-all` replaces the old built-in help; `task default` prints examples.

Startup sequence (the bootstrap at the bottom of `lib/common.sh`, sourced by every command script — replaced `infra.sh` + `lib/dispatch.sh` + `lib/env.sh` + `lib/run.sh`): exports `INFRA_ROOT` (pre-set values win — in-cluster pods ship it in their env), state dirs, `INFRA_INTERACTIVE` (tty detect), `TARGET` (unset = universal mode); target mode: if `hostname` ≠ target, warns (and prompts, when interactive); when a VARS file resolves (skipped for `compose generate-mtls-certs`), runs `lib/vars_validator.py <target>` and sources the env file it emits (`current_target/env.sh`, 0600); optional-load (never an error) for `validate`/`list-domains`/`k3s provision`/`compose backup-state`/universal aliases; hard-errors on missing VARS for render/apply commands (`compose install|restart|install-preboot`, `k3s deploy|group|update-node-ip|backup-pvc`, `restore-pvc`) when a template exists; sets `MY_UID` (1000 when root) always, `DOCKER_GID` + builtin-only `ENVSUBST_VARS` for compose renders (except `backup-state`/`generate-mtls-certs`). An exported `INFRA_ENV_LOADED` guard makes the bootstrap run once per top-level invocation. Universal mode (no `TARGET`) skips hostname check, target VARS, and compose setup entirely; the in-cluster CronJob/pod paths (no `TARGET` in their env) are untouched.
...
Place at `commands/<name>.sh` or `commands/<subdir>/<name>.sh`; target overrides at `targets/<t>/commands/...`. Then add a task entry to the root or the relevant `targets/<t>/Taskfile.yml` (`cmds: - TARGET=<t> INFRA_ROOT={{.ROOT_DIR}} bash {{.ROOT_DIR}}/<script> {{.CLI_ARGS}}`, `desc:` mirroring the DESC line). Useful helpers in `lib/`: `retry <tries> <delay> <cmd-string>`, `_confirm`, `get_sudo_cmd`, `wait_for_k3s_cluster`, `wait_for_crds <secs> <crd...>`, `get_node_ip`.

## Variables & templating

- **Two-layer vars**: `targets/<t>/VARS.template.env` (committed dotenv: placeholders + generation comments) vs. actual secrets in `secrets/VARS.<t>.env` (fallbacks in order: `secrets/VARS.env`, root `VARS.<t>.env`, root `VARS.env`). All gitignored. `lib/vars_validator.py` hard-fails if the real file is missing any template variable.
- **Format** — dotenv: `KEY="value"`, no `export`, no space before `=`. Multi-line values are real newlines inside the double quotes (indentation is data); escape only `\` → `\\` and `"` → `\"`. A bare `$` is literal — write `$argon2id$...` unescaped. References use `${NAME}` and must point at a variable defined **earlier in the same file** (unknown references are a hard error; the old bash format also expanded at source time, but in file order only). `#` inside quotes is literal; inline comments (` # ...`) work after single-line values. `python-dotenv` is the parser (`lib/vars_validator.py`; the old `.sh` files were evaluated bash and are gone — see the migration note below).
...
- **Validation happens in `lib/vars_validator.py` on every target-mode load** (invoked by the `lib/common.sh` bootstrap): template keys must all exist in the real file; placeholder values are rejected (`change_me`, `changeme`, `abc`, `REPLACE_ME`, `<...>`, trimmed, on template keys only); `${NAME}` must resolve. Errors print variable names only — **values never reach argv, stdout, or stderr**. `task <target>:validate` additionally checks YAML syntax, kustomize builds, and the rendered compose config.
- **`ENVSUBST_VARS`** — allowlist passed to envsubst, computed by `lib/vars_validator.py`: every variable in the real VARS file + built-ins (`MY_UID TARGET COMPOSE_STATE_DIR COMPOSE_STATE_BACKUP_DIR K3S_STATE_DIR PVC_BACKUP_DIR INFRA_ROOT DOCKER_GID PROXY_IP USER`) + derived `*_HASHED` names. Only known vars are expanded; leftovers surface as validate errors.
- **`*_HASHABLE` → `*_HASHED`** — auto-hashed with `openssl passwd -6 -stdin` (stdin pipe, never argv) at load time (e.g. Authelia OIDC client secrets: plaintext in VARS, hash mounted into Authelia). The plaintext `*_HASHABLE` stays in the environment (hooks like `immich/post.sh` consume it).
- **Multi-line vars** (Authelia user DB, JWKS, frigate config, geoblock subset, mTLS PEMs) — double-quoted dotenv values with literal indentation; whitespace is significant in the rendered YAML.
...
- Migration note: the old bash VARS (`VARS.*.sh`, evaluated with `source`) were converted with `migrate_vars.py` (deleted after use) — migrated values carry the *post-bash-expansion* text, so the `.env` files are byte-equivalent to what the old pipeline actually used. Possible future direction only, not planned: sops/age-encrypted VARS committed to git instead of gitignored dotenv.
...
- `compose backup-state` — local: tars `$COMPOSE_STATE_DIR` via an Alpine container (skips FIFOs/sockets). Remote: `compose backup-state <[user@]host:path> <remote_target>` SSHes into another Infra instance which streams the tar back (`INFRA_BACKUP_STREAM=true task <remote_target>:compose:backup-state` → stdout; the remote needs `task` installed). Prunes to `$BACKUP_RETENTION` (default 1).
- `compose generate-mtls-certs <server-target>` — per-pair CA, server cert (CN = server target), client cert, plus a preboot client cert when `MTLS_PREBOOT_CLIENT_CERT` is in the client's template. Prints dotenv copy-paste blocks for both VARS files; never edits VARS.
...
Node provisioning is Ansible, invoked through one Task entry point. There is **no inventory file anywhere** (committed or gitignored): the wrapper builds a throwaway inventory in `/tmp` from CLI args, so any node can become the control plane and any node can join in any role at runtime — no host metadata in the repo. Cluster policy defaults live in role `defaults/main.yml`; per-run values are extra vars from the CLI.
...
lib/                            # common.sh (index + env bootstrap) → pkg/net/k3s/compose/validate/pvc/mtls-certs
lib/vars_validator.py           # VARS validation/hashing → emits current_target/env.sh
...
- **Docker needs no manual sudo** — `lib/common.sh` defines a `docker()` wrapper that transparently retries with sudo when the docker socket is permission-denied (user not in the docker group on srv0/pc/bigpc), prompting for elevation instead of failing. Only daemon-permission errors trigger it; real CLI/daemon-down errors pass through.
