...
- **`k3s-server.sh`** (no args, on the node): runs `k3s-node-prep.sh` (sysctls in `/etc/sysctl.d/90-k3s.conf`; firewalld/ufw rules — cluster ports including 51820–21/udp for flannel-wg and 10250/tcp, which stays open because clustered Alloy may scrape a peer's kubelet, while 9100 is dropped since node-exporter is unauthenticated and scraped on the pod network; server nodes also get a `kubepods.slice` `IOWeight=10` and an idle-priority `fstrim.service` drop-in so pod I/O bursts and scheduled trims can't stall etcd), the installer, and `/etc/rancher/k3s/config.yaml` (`selinux: true`, `write-kubeconfig-mode: "0600"`, `flannel-backend: wireguard-native`, `cluster-init: true`, `node-ip`, plus `kube-controller-manager-arg`/`kube-scheduler-arg`/`kube-cloud-controller-manager-arg` lease tolerances so multi-second etcd write stalls don't exit k3s), then restarts, waits for the server token and the API, and applies the Longhorn default-disk label. Node feature labels are NFD-managed (see Security). K3s needs a fixed IP; if it changes, run `scripts/update-node-ip.sh`.
