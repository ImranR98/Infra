...
MAX_EXPAND_BYTES=$(numfmt --from=iec "${MAX_EXPAND// /}" 2>/dev/null || true)
if [ -z "$MAX_EXPAND_BYTES" ]; then
	echo "WARNING: could not parse max expandable size ($MAX_EXPAND) — skipping size guard" >&2
	MAX_EXPAND_BYTES=0
fi
...
SU=$(get_sudo_cmd)
$SU truncate -s "$NEW_SIZE_BYTES" "$HOST_PATH"
