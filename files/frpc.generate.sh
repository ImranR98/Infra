...
awk -v TOKEN="$TOKEN" '{gsub("put_token_here", TOKEN); print}' "$HERE_QC6O/$FRPC_FILENAME.template.toml" | awk -v TOKEN="$(echo $TOKEN | head -c 32)" '{gsub("put_password_here", TOKEN); print}' | envsubst | dd status=none of="$HERE_QC6O/$FRPC_FILENAME.toml"
