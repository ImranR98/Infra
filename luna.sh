...
EOF
    ...
    backup-state)
        ...
        if docker run --rm -v "$STATE_DIR":/backup/state:ro -v "$HERE/VARS.sh":/backup/VARS.sh:ro alpine sh -c 'apk add --no-cache tar >/dev/null 2>&1 && exec tar cf - --ignore-failed-read -C /backup .' > "$OUTPUT"; then
