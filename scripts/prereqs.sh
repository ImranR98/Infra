...
if [ -d "$target_dir/compose" ] && [ -f "$target_dir/compose/compose.yaml" ]; then
    ...
    env_args=()
    # The machine-fact .env (uid/gid/username) is written for EVERY target —
    ...
    # forced 1000 when running as root — containers must not run as 0), and
    # MY_USERNAME is that UID's login name (so running compose as root still
    # resolves the human user's home, e.g. /home/<user> binds).
    docker_gid="$(getent group docker | cut -d: -f3)"
    my_username="$(getent passwd "$MY_UID" 2>/dev/null | cut -d: -f1)"
    [ -n "$my_username" ] || my_username="${SUDO_USER:-${USER:-}}"
    {
