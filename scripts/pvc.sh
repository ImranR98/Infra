...
# ---- CLI dispatch ----------------------------------------------------------
# Usage: pvc.sh backup <pvc-name> | backup --all [-y] | restore <pvc-name> | restore --all [-y]
# NOTE: Does NOT scale down workloads on backup — captures live running state.

pvc_backup_one() {
    local pvc_name="$1" auto_yes="$2"
    local backup_file pvc_ns workloads timestamp exclude
...
pvc_restore_one() {
    local pvc_name="$1" auto_yes="$2"
    local backup_file pvc_ns workloads scaled_file timestamp
    backup_file="$PVC_BACKUP_DIR/${pvc_name}.tar.gz"
...
pvc_main() {
    ...
    case "$action" in
        backup|restore) ;;
        *)
            echo "Usage: $0 backup <pvc-name> | backup --all [-y] | restore <pvc-name> | restore --all [-y]" >&2
            exit 1
            ;;
    esac
    ...
    if [ "$action" = backup ]; then
        ...
        pvc_restore_one "$pvc_name" "$auto_yes"
    fi
}
