...
if bash "$INFRA_ROOT/lib/check_root_luks.sh"; then
    echo "LUKS detected. Installing crypt-ssh remote unlock (port 8887)..."

    TMPDIR="$(mktemp -d)"
    trap 'rm -rf "$TMPDIR"' EXIT
    git clone --depth 1 https://github.com/ImranR98/dracut-remote-luks-unlock.git "$TMPDIR"
    # Direct LAN access (no FRP tunnel): make the initramfs SSH listen on 8887
    sed -i 's/dropbear_port="22"/dropbear_port="8887"/' "$TMPDIR/modules/99crypt-ssh/crypt-ssh.conf"
    ...
    echo "listening on port 8887 (direct LAN connection, no FRP tunnel)."
else
